...
export default function TravelPolish() {
  ...
  useEffect(() => {
    const stored = Number(window.localStorage.getItem("travel-polish") ?? "0");
    if (!Number.isNaN(stored)) setI(stored % VARIANTS.length);
  }, []);
  useEffect(() => {
    window.localStorage.setItem("travel-polish", String(i));
  }, [i]);
            ...
            >
              {VARIANTS.map((v, idx) => (
                ...
                >
                  ...
                  <span className="font-display text-body text-primary w-28">{v.name}</span>
